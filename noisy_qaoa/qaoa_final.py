...
Original file is located at
    https://colab.research.google.com/drive/1LfIdRxusD3C5ZCOqxHVmJzQO7RqOgW2_
"""
...
def get_noisy_circuit(params, noise_param):
    ...
    for op in compile_tape:
        qml.apply(op)
        if op.name == 'CNOT':
            qml.DepolarizingChannel(noise_param, op.wires[1])
...
def qaoa_circuit(params, noise_param):
    ...
    for wire in range(num_wires):
        qml.RZ(2*np.pi, wires=wire)
        qml.RY(np.pi/2, wires=wire)
    get_noisy_circuit(params, noise_param)
...
qaoa_circuit()
#print(tape.operations[0])

for op in qaoa_circuit.tape:
    print(op)
